...
router.post('/login', validateLoginInputs, validateErrors, getUser, validateUser, function (req, res) {
	var token = jwt.sign({
		user: req.user
	}, 'ghostrider', {
		expiresIn: '1000h'
...
router.post('/register', validateRegisterErrors, validateErrors, appendNewUserObj,
	ensureUniqueUsername, ensureUniqueEmail, createNewUser,
	function (req, res) {
		res.status(200).json({
...
// use the new token!
router.put('/me', ensureAuthenticatedApi, validateUserBody, validateErrors, function (req, res) {
...
function modifyUserStickers(req, res, next) {
	...
	req.user.stickers = req.user.stickers.filter(x => !req.body.remove.includes(x));
...
router.put('/me/stickers', ensureAuthenticatedApi, fetchUser, validateStickersArray, validateErrors, modifyUserStickers, function (req, res) {
	res.status(200).json({
		user: req.user
	});
});
